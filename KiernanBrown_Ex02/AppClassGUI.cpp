...
#pragma endregion
	...
	if (m_bGUI_Main)
	{
		...
		ImGui::Begin(sAbout.c_str(), (bool*)0, window_flags);
		{
			...
			ImGui::Text("O: Toggle Plane Overlay\n");
			ImGui::Text("Arrows: Change selected node\n");
			ImGui::Text("+: Increase terrain cost of selected node (Max 3)\n");
			ImGui::Text("-: Decrease terrain cost of selected node (Min 1)\n");
			ImGui::Text("T: Toggle walkability of selected node\n");
			ImGui::Text("P: Move Player to the selected node\n");
			ImGui::Text("R: Reset Enemies to their starting positions\n");
			ImGui::Text("Spacebar: Start Pathfinding for all Enemies\n");
		}
		ImGui::End();
	}
