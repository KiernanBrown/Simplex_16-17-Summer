...
class MyEntity
{
	...
	SystemSingleton* m_pSystem = nullptr; // Singleton of the system, for following a path
	...
	std::vector<MapNode*> m_pPathList;
	MapNode* m_pCurrentNode = nullptr;		// MapNode this entity is currently on
...
public:
	...
	*/
	...
	float m_fPathTimerCount = 0.0f;
	uint m_uClock = 0;
	void FollowPath(std::vector<MapNode*> a_pPath, float a_fPathTime);
	void SetCurrentNode(MapNode* a_pMapNode);
