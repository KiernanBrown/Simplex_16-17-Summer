...
void Application::ProcessKeyPressed(sf::Event a_event)
{
	...
	// Toggle Overlay
	case sf::Keyboard::O:
		m_bPlaneOverlay = !m_bPlaneOverlay;
		break;

	// Toggle Walkability of Selected Node
	case sf::Keyboard::T:
		m_pSelectedNode->SetWalkable(!m_pSelectedNode->IsWalkable());
		break;

	// Increase Terrain Cost of Selected Node
	case sf::Keyboard::Add:
		if (m_pSelectedNode->GetTerrainCost() < 3.0f)
		{
			m_pSelectedNode->SetTerrainCost(m_pSelectedNode->GetTerrainCost() + 1.0f);
		}
		break;

	// Decrease Terrain Cost of Selected Node
	case sf::Keyboard::Subtract:
		if (m_pSelectedNode->GetTerrainCost() > 1.0f)
		{
			m_pSelectedNode->SetTerrainCost(m_pSelectedNode->GetTerrainCost() - 1.0f);
		}
		break;
	...
	case sf::Keyboard::P:
		m_pEntityMngr->GetEntity(m_pEntityMngr->GetEntityIndex("Player"))->SetCurrentNode(m_pSelectedNode);
		break;

	// Arrow Movement of Selected Node
	// The direction adjusts (for the most part, going between elevations gets a bit weird) based on the camera
	case sf::Keyboard::Up:
		{
			m_v3NewSelectedPos = m_pSelectedNode->GetCenter() + m_pCameraMngr->GetCamera(0)->GetForward() * 7.0f;
			float lowestDistance = FLT_MAX;
			MapNode* newSelectedNode = nullptr;
			for (int i = 0; i < m_pSelectedNode->GetNeighbors().size(); i++)
			{
				vector3 neighborPos = vector3(m_pSelectedNode->GetNeighbors()[i]->GetCenter().x, 0, m_pSelectedNode->GetNeighbors()[i]->GetCenter().z);
				if (glm::distance(m_v3NewSelectedPos, neighborPos) < lowestDistance)
				{
					lowestDistance = glm::distance(m_v3NewSelectedPos, m_pSelectedNode->GetNeighbors()[i]->GetCenter());
					newSelectedNode = m_pSelectedNode->GetNeighbors()[i];
				}
			}
			if (newSelectedNode != nullptr)
			{
				m_pSelectedNode = newSelectedNode;
			}
		}
		break;
	...
	case sf::Keyboard::Left:
		{
			m_v3NewSelectedPos = m_pSelectedNode->GetCenter() - m_pCameraMngr->GetCamera(0)->GetRightward() * 7.0f;
			float lowestDistance = FLT_MAX;
			MapNode* newSelectedNode = nullptr;
			for (int i = 0; i < m_pSelectedNode->GetNeighbors().size(); i++)
			{
	...
	case sf::Keyboard::Space:
		// Pressing Space makes all of the enemies follow a path towards the player
		MyEntity* player = m_pEntityMngr->GetEntity(m_pEntityMngr->GetEntityIndex("Player"));
		if (player != nullptr)
		{
			for (int i = 1; i < 4; i++)
			{
				MyEntity* enemy = m_pEntityMngr->GetEntity(m_pEntityMngr->GetEntityIndex("Enemy_" + std::to_string(i)));

				// Each enemy will follow a path from their current node to the player's current node, taking 0.5 seconds between nodes
				if (enemy != nullptr)
				{
					// Only start pathfollowing if you are not following a path yet
					if (enemy->m_path == -1)
					{
						enemy->FollowPath(m_pPathfinder->FindPath(enemy->GetCurrentNode(), player->GetCurrentNode()), 0.5f);
					}
				}
			}
		}
