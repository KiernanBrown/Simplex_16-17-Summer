...
void Application::ReadConfig(void)
{
	...
	while (reader.ReadNextLine() == OUT_RUNNING)
	{
		...
		else if (sWord == "Rotating:")
		{
			int nValue;
			sscanf_s(reader.m_sLine.c_str(), "Rotating: %d", &nValue);
			rotating = nValue;
		}
...
void Application::WriteConfig(void)
{
	...
	// Add our Rotating value to the configuration file
	fprintf(pFile, "\n\n//Option created by Kiernan Brown. Set to 1 to have the torus rotate 90 degrees when the sphere on the torus completes its orbit.");
	fprintf(pFile, "\nRotating: %d", rotating);
