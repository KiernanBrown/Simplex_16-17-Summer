...
void Application::CameraRotation(float a_fSpeed)
{
	...
	// Our ChangeYawPitchRoll method will change the direction we face and set the target
	// Multiply by 4.5 in order to make the camera rotate faster
	m_pCamera->ChangeYawPitchRoll(vector3(-fAngleY * 4.5f, -fAngleX * 4.5f, 0.0f));
...
#pragma region Camera Position
	...
	{
		// Move forward
		// We adjust our position and target's position by using our direction and the speed we are moving at
	...
	{
		// Move to the right at negative speed, which is moving to the left
	...
	{
		// Move in our direction at negative speed, which is moving to the backwards
		m_pCamera->SetPosition(m_pCamera->GetPosition() + m_pCamera->GetDirection() * -fSpeed);
		m_pCamera->SetTarget(m_pCamera->GetTarget() + m_pCamera->GetDirection() * -fSpeed);
	}
	...
	// Adjust our direction and right vector
	// Our direction is the target's position minus our position
	// Our right vector is the cross product of our direction and the vector pointing up
