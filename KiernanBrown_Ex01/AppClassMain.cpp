...
void Application::ReadConfig(void)
{
	...
	while (reader.ReadNextLine() == OUT_RUNNING)
	{
		...
		// Code to add a stop from the configuration file
		else if (sWord == "Stop:")
		{
			float fValueX;
			float fValueY;
			float fValueZ;
			sscanf_s(reader.m_sLine.c_str(), "Stop: [%f,%f,%f]", &fValueX, &fValueY, &fValueZ);
			m_stopsList.push_back(vector3(fValueX, fValueY, fValueZ));
		}
...
void Application::WriteConfig(void)
{
	...
	{
		fprintf(pFile, "\nStop: [%.3f,%.3f,%.3f]",
			m_stopsList[i].x, m_stopsList[i].y, m_stopsList[i].z);
	}
